...
#define AS7341_SPECTRAL_INT_LOW_MSK                                            \
  0b00010000 ///< bitmask to check for a low threshold interrupt
	...
	typedef enum {
		...
		bool Adafruit_AS7341_begin(uint8_t i2c_addr, I2C_HandleTypeDef *i2c_handle,
				int32_t sensor_id);
		...
		void Adafruit_AS7341_setup_F1F4_Clear_NIR(void);
		void Adafruit_AS7341_setup_F5F8_Clear_NIR(void);
		...
		bool Adafruit_AS7341_setSpectralThresholdChannel(as7341_adc_channel_t channel);
		...
		bool Adafruit_AS7341_setBank(bool low); // low true gives access to 0x60 to 0x74
		...
		bool Adafruit_AS7341_setGPIOValue(bool);
		...
		void Adafruit_AS7341_FDConfig(void);
		int8_t Adafruit_AS7341_getFlickerDetectStatus(void);
		bool Adafruit_AS7341_setSMUXCommand(as7341_smux_cmd_t command);
		bool Adafruit_AS7341_writeRegister(uint8_t mem_addr, uint8_t *val, uint16_t size);
		...
		bool Adafruit_AS7341_modifyRegisterMultipleBit(uint16_t reg, uint8_t value, uint8_t pos, uint8_t bits);

		bool Adafruit_AS7341_readRegister(uint16_t mem_addr, uint8_t *dest, uint16_t size);
